// Dev-mode mock result generator for when VERIFICATION_WORKER_URL is not set.
// Simulates a worker callback by generating a mock VerificationResult.

export function generateMockResult(verificationId: string, fileName: string) {
  const name = fileName.toLowerCase();

  if (name.includes('tamper')) {
    return mockTamperedResult(verificationId);
  } else if (name.includes('unsigned')) {
    return mockUnsignedResult(verificationId);
  }
  return mockAuthenticResult(verificationId);
}

function mockAuthenticResult(verificationId: string) {
  return {
...
function mockTamperedResult(verificationId: string) {
  return {
    verification_id: verificationId,
...
function mockUnsignedResult(verificationId: string) {
  return {
    verification_id: verificationId,
