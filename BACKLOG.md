...
## Ready
- [ ] Close the worker verification gap in `CLAUDE.md`: add `worker/Makefile` with a `.venv` file target (`uv venv --python 3.12 --seed .venv && .venv/bin/pip install -r requirements.txt`; must NOT re-run when `.venv` exists — it is pre-built, you are offline) and a `test` target depending on it that runs `.venv/bin/pytest -m "not linux_sandbox"` (linux_sandbox tests fail off-Linux by design; they stay on the Docker gate). Document both in `CLAUDE.md` Verification — accept: `make -C worker test` exits 0 offline on this Mac; no changes under `src/`.
- [ ] `worker/app/sandbox.py`: add a unit test that `run_sandboxed` rejects an upload over the configured size limit before spawning ffprobe — accept: `make -C worker test` exits 0 with the new test (Mac selection, `-m "not linux_sandbox"`).
